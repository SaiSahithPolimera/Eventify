...
const AdminDashboard = () => {
  ...
  const renderStatCard = (label, value, bgColor, textColor) => (
    <div
      className={`bg-gradient-to-br ${bgColor} rounded-lg p-4 sm:p-5 border border-opacity-30 ${textColor.replace(
        "text-",
        "border-"
      )}`}
    >
  ...
  const renderEventSummary = () => (
    <div className="mb-6 sm:mb-8 bg-white rounded-lg border border-slate-200 p-4 sm:p-6">
      <h2 className="text-xl sm:text-2xl font-bold text-slate-900 mb-1 sm:mb-2 line-clamp-2">
        {selectedEvent?.title}
      </h2>
    ...
    <div>
      ...
      <div className="lg:hidden p-3 sm:p-4 border-b border-slate-200 bg-slate-50">
        <button
          ...
          className="w-full flex items-center justify-between text-slate-900 font-semibold hover:text-slate-700 transition"
        >
      ...
      {/* Filters (visible on lg+, or on mobile when toggled) */}
      <div
        className={`${mobileFiltersOpen ? "block" : "hidden lg:block"
          } p-4 sm:p-6 border-b border-slate-200 bg-slate-50 transition-all`}
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-3 sm:mb-4">
          <div>
            ...
            <input
              ...
              className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-rose-500 text-xs sm:text-sm"
            />
          </div>
        ...
        <button
          ...
          className="text-xs sm:text-sm font-semibold text-slate-600 hover:text-slate-900 underline"
        >
          Clear All Filters
        </button>
      </div>
          ...
          <table className="w-full text-xs sm:text-sm">
            <thead>
              <tr className="bg-slate-100 border-b border-slate-200">
                ...
                <th className="px-3 sm:px-4 py-2 sm:py-3 text-left font-semibold text-slate-900 hidden sm:table-cell">
                  Email
                </th>
                <th className="px-3 sm:px-4 py-2 sm:py-3 text-left font-semibold text-slate-900 hidden lg:table-cell">
                  Role
                </th>
                <th className="px-3 sm:px-4 py-2 sm:py-3 text-left font-semibold text-slate-900">
                  Status
                ...
                >
                  <td className="px-3 sm:px-4 py-3 sm:py-4 font-medium text-slate-900 text-xs sm:text-sm">
                    ...
                    <a
                      ...
                      className="text-rose-600 hover:text-rose-700 underline text-xs sm:hidden"
                  ...
                  <td className="px-3 sm:px-4 py-3 sm:py-4 text-slate-600 hidden sm:table-cell">
                    <a
                      href={`mailto:${attendee.email}`}
                      className="text-rose-600 hover:text-rose-700 underline truncate block text-xs sm:text-sm"
                      title={`Send email to ${attendee.email}`}
                    >
                      {attendee.email}
                    </a>
                  ...
                  <td className="px-3 sm:px-4 py-3 sm:py-4 hidden lg:table-cell">
                    <span
                      className={`px-2 sm:px-3 py-1 rounded-full text-xs font-semibold inline-block ${attendee.role === "organizer"
                  ...
                  <td className="px-3 sm:px-4 py-3 sm:py-4">
                    <span
                      className={`px-2 sm:px-3 py-1 rounded-full text-xs font-semibold inline-block ${attendee.status === "confirmed"
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="w-full px-4 sm:px-6 py-6 sm:py-8 max-w-7xl mx-auto">
        <div className="mb-6 sm:mb-8">
          <h1 className="text-3xl sm:text-4xl text-slate-900 mb-1 sm:mb-2 font-sans line-clamp-1">
            Admin Dashboard
          </h1>
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <select
            ...
            className="w-full border border-slate-300 bg-white hover:rounded-lg px-3 sm:px-4 py-2 sm:py-3 focus:outline-none focus:ring-2 focus:ring-rose-500 text-slate-900 font-medium text-sm sm:text-base"
          >
        ...
        {selectedEventId && selectedEvent && stats ? (
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between gap-4 p-4 sm:p-6 border border-slate-200 rounded-lg bg-white">
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                <button
                  ...
                  onClick={exportToCSV}
                  className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-semibold transition cursor-pointer flex items-center justify-center gap-2 ${filteredAttendees.length === 0
          ...
          <div className="text-center py-12 sm:py-20 bg-white rounded-lg border border-slate-200">
            ...
            <p className="text-slate-600 text-sm sm:text-lg px-4">
              Choose an event from the dropdown above to view all attendees and analytics
            </p>
          </div>
        )}
      </div>
